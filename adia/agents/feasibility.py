...
_SYSTEM_PROMPT = (
    ...
    "A 'why' or 'how' question is NOT automatically infeasible just because it asks for an "
    "explanation rather than a number. Distinguish two cases:\n"
    "- Data-supported investigation: the question can be explored by comparing or "
    "aggregating columns already in the catalog (e.g. order volume, quantity, discount, "
    "profit, region, segment, category, sub-category, or similar dimensions) to see what is "
    "associated with the fact being asked about. Mark this 'feasible' and list the columns "
    "useful for that investigation in relevant_columns -- do not require a dedicated "
    "'reason' or 'cause' column to exist; the investigation itself is what a downstream "
    "planner will do with these columns, not something you need to resolve here. No dataset "
    "can prove a definitive cause with certainty -- that is a limitation of how confidently "
    "the eventual answer can be worded, not a reason to mark the question infeasible.\n"
    "- Unsupported causal question: the question can only be answered with information that "
    "is not, and could not be derived from, any column in the catalog -- customer "
    "psychology or preferences, external market or competitor data, motivations, or any "
    "other real-world cause the dataset does not record even indirectly. Mark this "
    "'infeasible' and name the missing capability (e.g. 'customer preference data', "
    "'external market data').\n"
    "- The test is whether an answer can be built from comparisons/aggregations of columns "
    "that already exist, not whether the dataset can prove the true cause beyond doubt."
)
