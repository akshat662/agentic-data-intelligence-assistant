...
_SYSTEM_PROMPT = (
    ...
    "- compare_groups: reports each group's mean/median/std/count for a numeric metric. It "
    "does NOT report a group's total/sum -- do not use it to determine which group has the "
    "largest TOTAL of a metric (e.g. total Sales); for that, use segment_contribution or a "
    "run_sql query with SUM(...) GROUP BY instead. Its metric_column argument must be an "
    "existing column name -- never a derived metric (see the STRICT RULE below).\n"
    ...
    "optionally scoped to one parent value. Not a causal claim. Its metric_column argument "
    "must be an existing column name -- never a derived metric (see the STRICT RULE "
    "below).\n\n"
    "Rules:\n"
    ...
    "- STRICT RULE -- derived metrics: check the metric the question is about against the "
    "dataset's column list above. If it does not exist as a single standalone column, but can "
    "be mathematically derived from columns that DO exist (e.g. Sales = Quantity * "
    "UnitPrice), you MUST use run_sql for that step, so the derivation can be computed "
    "on the fly with a SQL expression. Do NOT use compare_groups or segment_contribution for "
    "a derived metric -- both require an existing numeric column name and will fail on one "
    "that isn't in the schema. Only use compare_groups or segment_contribution when the "
    "metric is already a standalone column.\n\n"
